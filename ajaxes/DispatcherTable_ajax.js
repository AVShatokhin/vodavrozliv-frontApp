...
let getDispatcherHistory = (component, data, when_CB, catch_CB) => {
  let token = JSON.parse(localStorage.getItem("userData"))?.token;
  data.token = token;
  ...
  component
    .axios({
      method: "POST",
      timeout: 15000,
      url: `${api_url}getDispatcherHistory/`,
      data,
    })
    .then((response) => {
      when_CB(response.data);
...
let getDispatcherHistory_XML = async (component, data, when_CB, catch_CB) => {
  let token = JSON.parse(localStorage.getItem("userData"))?.token;
  data.token = token;
  ...
  await component
    .axios({
      method: "POST",
      timeout: 15000,
      url: `${api_url}getDispatcherHistory_XML/`,
      data,
    })
    .then((response) => {
      when_CB(response.data);
